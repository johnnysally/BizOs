...
async function enforceOwnerLimit(tenantId, newRole) {
  if (newRole !== 'owner') return;

  const tenant = await Tenant.findById(tenantId).lean();
  const plan = await Plan.findOne({ code: tenant?.planId || 'free' }).lean();
  const max = plan?.limits?.maxOwners || 3;

  const count = await User.countDocuments({ tenantId, role: 'owner', status: 'active' });
  if (count >= max) throw ApiError.badRequest('OWNER_LIMIT', `Max ${max} owners allowed`);
}
...
const invite = asyncHandler(async (req, res) => {
  ...
  await enforceOwnerLimit(req.tenantId, role);
...
const updateRole = asyncHandler(async (req, res) => {
  ...
  if (role === 'owner') await enforceOwnerLimit(req.tenantId, role);
