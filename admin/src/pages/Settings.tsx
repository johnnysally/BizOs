...
import type { PlatformSettings, FeatureFlags } from '@/api/settings';
import { DownloadsTab } from '@/components/admin/settings/DownloadsTab';
...
export default function Settings() {
  ...
  useEffect(() => {
    Promise.all([settingsApi.get(), settingsApi.getFeatures()])
      ...
      .catch((err) => {
        toast({
          type: 'error',
          message: (err as { message?: string }).message || 'Failed to load settings',
        });
      })
  ...
  const isDownloadsTab = tab === 'downloads';
  ...
  return (
    <div className="space-y-6">
        ...
        <Card title="General">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <FormField label="Website">
              <Input
                value={(settings.platform_website as string) || ''}
                onChange={(e) => patch('platform_website', e.target.value)}
                placeholder="https://bizos.co.ke"
              />
            </FormField>
            ...
            <FormField label="Support email">
              <Input
                ...
                placeholder="support@bizos.co.ke"
              />
            </FormField>

            <FormField label="Support phone">
              <Input
                value={(settings.support_phone as string) || ''}
            ...
            <FormField
              label="Logo URL"
              hint="Cloudinary or any HTTPS URL"
              className="md:col-span-2"
            >
              <Input
                value={(settings.platform_logo_url as string) || ''}
        ...
        <Card title="Defaults for new tenants">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField label="Default currency">
              <Input
                value={(settings.default_currency as string) || ''}
            ...
            <FormField label="Default country code">
              <Input
                value={(settings.default_country as string) || ''}
            ...
            <FormField label="Default tax rate (%)">
              <Input
                ...
                onChange={(e) =>
                  patch('default_tax_rate', Number(e.target.value))
                }
              />
            </FormField>
            ...
            <FormField label="Minimum password length">
              <Input
                type="number"
                value={
                  settings.min_password_length !== undefined
                    ? Number(settings.min_password_length)
                    : 8
                }
                onChange={(e) =>
                  patch('min_password_length', Number(e.target.value))
                }
              />
            </FormField>

            <FormField label="Tax inclusive by default">
              <label className="flex items-center gap-2 mt-2 text-sm">
                <input
                  type="checkbox"
                  checked={settings.tax_inclusive === true}
                  onChange={(e) => patch('tax_inclusive', e.target.checked)}
            ...
            {Object.entries(features).map(([key, value]) => (
              ...
              >
                ...
                <input
                  ...
                  checked={value}
                  onChange={(e) =>
                    setFeatures({ ...features, [key]: e.target.checked } as FeatureFlags)
                  }
                  className="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500"
                />
              </label>
          ...
          <div className="space-y-3">
            ...
            <label className="flex items-center justify-between gap-4 py-2 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Block new registrations and client API access
                </p>
              </div>
              <input
                type="checkbox"
      ...
      {/* Limits */}
      {tab === 'limits' && (
        <Card title="User and cashier limits">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField label="Max owners per tenant">
              <Input
                type="number"
                value={
                  settings.max_owners_per_tenant !== undefined
                    ? Number(settings.max_owners_per_tenant)
                    : 3
                }
                onChange={(e) =>
                  patch('max_owners_per_tenant', Number(e.target.value))
                }
              />
            </FormField>
            ...
            <FormField label="Cashier discount limit (%)">
              <Input
                type="number"
                value={
                  settings.cashier_discount_limit !== undefined
                    ? Number(settings.cashier_discount_limit)
                    : 10
                }
                onChange={(e) =>
            ...
            <FormField label="Cashier refund limit">
              <Input
                type="number"
                value={
                  settings.cashier_refund_limit !== undefined
                    ? Number(settings.cashier_refund_limit)
                    : 0
                }
                onChange={(e) =>
            ...
            <FormField label="Manager can invite cashiers">
              <label className="flex items-center gap-2 mt-2 text-sm">
                <input
                  type="checkbox"
                  checked={settings.manager_can_invite_cashier === true}
                  onChange={(e) =>
                    patch('manager_can_invite_cashier', e.target.checked)
                  }
                  className="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500"
                />
                <span>Allow managers to add cashiers</span>
              </label>
            </FormField>

            <FormField label="Require shift clock-in">
              <label className="flex items-center gap-2 mt-2 text-sm">
                <input
                  type="checkbox"
                  checked={settings.require_shift_clock_in === true}
                  onChange={(e) =>
                    patch('require_shift_clock_in', e.target.checked)
                  }
                  className="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500"
                />
                <span>Cashiers must clock in before selling</span>
              </label>
            </FormField>
          </div>
        </Card>
