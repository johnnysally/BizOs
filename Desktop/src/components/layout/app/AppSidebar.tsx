...
export function AppSidebar() {
  ...
  return (
    <aside className="hidden md:flex w-64 shrink-0 h-full flex-col border-r border-border bg-surface">
      <div className="h-16 shrink-0 flex items-center gap-3 px-5 border-b border-border">
        {settings?.logoUrl ? (
        ...
        <div>
          <p className="text-sm font-semibold text-fg leading-none truncate max-w-[172px]">
            {tenant?.name || 'BizOS'}
          </p>
                      ...
                      <>
                        <span
                          className={classNames(
                            'absolute left-0 top-1/2 -translate-y-1/2 w-0.5 rounded-r-full bg-brand-600 transition-all',
      ...
      <div className="shrink-0 border-t border-border p-3">
        <div className="flex items-center gap-3 rounded-lg bg-bg px-3 py-2.5">
          <div className="w-8 h-8 shrink-0 rounded-full bg-brand-100 text-brand-700 dark:bg-brand-500/15 dark:text-brand-300 flex items-center justify-center text-xs font-semibold">
            {user ? initials(user.fullName) : '?'}
          </div>
          <div className="min-w-0">
            <p className="text-xs font-semibold text-fg truncate">{user?.fullName || 'Account'}</p>
            <p className="text-[11px] text-muted capitalize truncate">{role}</p>
          </div>
        </div>
      </div>
    </aside>
